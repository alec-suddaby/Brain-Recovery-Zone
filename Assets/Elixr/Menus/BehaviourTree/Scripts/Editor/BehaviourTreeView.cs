    ...
    public class BehaviourTreeView : GraphView
    {
        ...
        public override void BuildContextualMenu(ContextualMenuPopulateEvent evt)
        {
            ...
            Vector2 nodePosition = this.ChangeCoordinatesTo(contentViewContainer, evt.localMousePosition);
            {
                ...
                {
                    evt.menu.AppendAction($"Action Menu/{type.Name}", (a) => CreateNode(type, nodePosition));
                }
            }

            {
                ...
                foreach (var type in types)
                {
                    evt.menu.AppendAction($"Composite Menu/{type.Name}", (a) => CreateNode(type, nodePosition));
                }
            }
        ...
        private void DuplicateNode(DropdownMenuAction obj)
        {
            selection.ForEach(selectedItem =>
            {
                if (selectedItem is NodeView selectedNode)
                {

                    Node node = serializer.DuplicateNode(selectedNode.node);
                    CreateNodeView(node);
                }
            });
