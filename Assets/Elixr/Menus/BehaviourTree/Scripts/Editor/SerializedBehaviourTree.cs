...
namespace Elixr.MenuSystem
{
    ...
    public class SerializedBehaviourTree
    {
        ...
        public Node DuplicateNodeInstance(Node originalNode)
        {

            Node node = originalNode.Clone() as Node;
            node.position += new Vector2(40, 40);
            node.guid = GUID.Generate().ToString();
            return node;
        }
        ...
        public Node DuplicateNode(Node originalNode)
        {

            Node node = DuplicateNodeInstance(originalNode);
